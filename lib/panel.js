 ...
 */
import { fetchPlan, fetchUsageLimits, fetchUsageWindows } from './cline.js'
...
/**
 * The instant one quota window began.
 *
 * `resetsAt` is the END of a calendar-aligned period, so the start is that
 * instant minus exactly one period. Months subtract as a calendar month: a fixed
 * 30 days would drift off the period the gateway actually measured.
 *
 * @returns epoch milliseconds, or NaN when the window is not one this build knows.
 */
export function windowStartMs(limit) {
  const end = Date.parse(String(limit?.resetsAt ?? ''))
  if (!Number.isFinite(end)) return Number.NaN
  const type = String(limit?.type ?? '')
  if (type === 'five_hour') return end - 5 * 3_600_000
  if (type === 'weekly') return end - 7 * 86_400_000
  if (type === 'monthly') {
...
export function createPanel({ control, engine, store }) {
  ...
  async function state() {
    ...
    return {
      ...
      // The last readings, never requests: refreshing quota is its own action.
      usage: store.usage(),
      usageWindows: store.usageWindows(),
      plan: store.plan(),
  ...
  async function usage() {
    ...
    const readings = await Promise.all(accounts.map(async (account) => {
      if (account.keyConfigured !== true) return { account: account.name, ok: false, limits: [], error: `no API key stored for ${account.name}` }
  ...
  /**
   * Read what each quota window cost and consumed, for every enabled account.
   *
   * The windows are measured in money, so this reports money: the plan names the
   * USD cap and this sums what the usage history says was spent against it. The
   * window reading costs a few requests per account (a row walk for the five-hour
   * window, one daily aggregation each for the week and the month), which is why
   * it is a separate, cached action from {@link usage}.
   */
  async function usageWindows() {
    const config = readConfig()
    const accounts = await control.usageAccounts()
    const limitsOf = (name) => (store.usage().accounts ?? []).find((entry) => entry.account === name)?.limits ?? []
    let asked = false
    const readings = await Promise.all(accounts.map(async (account) => {
      const windows = limitsOf(account.name)
        .map((limit) => ({ type: String(limit.type), startMs: windowStartMs(limit), endMs: Date.parse(String(limit.resetsAt ?? '')) }))
        .filter((window) => Number.isFinite(window.startMs))
      if (account.keyConfigured !== true || windows.length === 0) {
        return { account: account.name, ok: false, windows: [], pages: 0, partial: false, error: 'read this account’s quota windows first' }
      }
      asked = true
      const result = await fetchUsageWindows({
        baseURL: account.baseURL || String(config.baseURL ?? ''),
        apiKey: account.key,
        windows,
      })
    ...
    if (!asked) throw new Error('read the quota windows first: there is no period to sum over')
    const reading = store.setUsageWindows({ ok: readings.some((entry) => entry.ok === true), accounts: readings })
    return { ...(await state()), usageWindows: reading }
  }

  /**
   * Read every enabled account's plan, which names the USD cap behind each
   * quota percentage.
   *
   * One cheap request per account, cached: without it a percentage has no scale,
   * which is exactly the question the panel could not answer before.
   */
  async function usagePlan() {
    const config = readConfig()
    const accounts = await control.usageAccounts()
    const readings = await Promise.all(accounts.map(async (account) => {
      if (account.keyConfigured !== true) return { account: account.name, ok: false, caps: {}, planName: '', error: `no API key stored for ${account.name}` }
    ...
    const plan = store.setPlan({ accounts: readings })
    return { ...(await state()), plan }
  }
