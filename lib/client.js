...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // medium triple instead. Destructuring a single name yields `undefined` on
    // the other host, and React throws on an undefined element type — which
    // takes the whole panel down, not just the chevron. Resolve whichever this
    // host publishes, and let `Chevron` fall back to an inline glyph.
    const PRIMITIVES_SPECIFIER = '@deepseek-ai/dsh-client-ui-primitives'
    const primitives = require(PRIMITIVES_SPECIFIER)
    const HostChevron = primitives.IconChevronDownOutline14
    ...
    const CSS = [
      ...
      '.cp-history-table{width:100%;border-collapse:collapse;table-layout:fixed}',
      ...
      '.cp-history-col-dot{width:18px}',
      '.cp-history-col-when{width:62px}',
      '.cp-history-col-tokens{width:120px}',
      '.cp-history-col-load{width:108px}',
      ...
      '.cp-history-load-row{display:flex;justify-content:space-between;gap:6px;font-size:11.5px;line-height:1.5}',
      ...
      '.cp-usage-group+.cp-usage-group{border-top:0.5px solid var(--dsw-alias-border-l2);padding-top:10px}',
      '.cp-usage-account{font-size:12px;font-weight:600;color:var(--dsw-alias-label-secondary);margin-bottom:6px}',
    ...
    const ZH = {
      ...
      usageMeasure: '统计用量',
      usageMeasuring: '统计中…',
      usageSpend: '已用 {used} / 额度 {cap}',
      usageSpendHint: '按官方已用百分比与套餐额度换算，仅供金额参考',
      usageMeasured: '明细核对 {cost} · {tokens} tokens · {n}',
      usageMeasuredHint: '由官方用量日志汇总；与上面的金额略有差异属正常',
      usageRequests: '{n} 次请求',
      usageDaysSummed: '{n} 天汇总',
      usageExact: '逐条统计',
      usageDaily: '按日统计',
      usageNoPlan: '套餐未给出额度上限',
      usageNoDetail: '明细暂不可用（网关本次未返回数据）',
      usageByModel: '按模型',
      usageByModelTop: '前 {n} / 共 {total}',
      historyScope: '仅本进程内记录，宿主重启后重新计数',
      historyWhen: '时间',
      historyModel: '模型',
      historyUpstream: '上游',
      historyLoad: '延迟',
      ...
      historyEffort: '推理强度',
      historyStats: '共 {n} 条 · 成功 {ok} · 失败 {bad}',
      historyStatsClean: '共 {n} 条 · 全部成功',
      historyAuto: '(自动)',
      noChannels: '未探测渠道',
      pinned: '已钉住',
      auto: '自动路由',
      autoSetup: '一键配置',
      autoSetupHint: '一键配置＝探测渠道 → 逐渠道校验可用性 → 钉住可用渠道 → 发一次真实请求验证',
      probe: '探测',
      validate: '校验',
      testPin: '测试',
      reset: '恢复自动',
    ...
    const EN = {
      ...
      usageSpend: 'used {used} of {cap}',
      usageSpendHint: 'converted from the official percentage and the plan’s quota cap; a reference figure only',
      usageMeasured: 'log check {cost} · {tokens} tokens · {n}',
      usageMeasuredHint: 'summed from the official usage log; a small difference from the figure above is expected',
      usageRequests: '{n} request(s)',
      usageDaysSummed: '{n} day(s) summed',
      usageExact: 'counted per request',
      usageDaily: 'counted per day',
      usageNoPlan: 'the plan names no quota cap',
      usageNoDetail: 'detail unavailable — the gateway returned no rows this time',
      usageByModel: 'By model',
      usageByModelTop: 'top {n} of {total}',
      historyScope: 'this host process only — a restart starts the count over',
      historyWhen: 'Time',
      historyModel: 'Model',
      ...
      historyStats: '{n} rows · {ok} ok · {bad} failed',
      historyStatsClean: '{n} rows · all succeeded',
      historyAuto: '(auto)',
      noChannels: 'No channels probed',
      pinned: 'Pinned',
      auto: 'Auto',
      autoSetup: 'Auto-configure',
      autoSetupHint: 'Auto-configure = probe channels → validate each one → pin the available ones → verify with one real request',
      probe: 'Probe',
      validate: 'Validate',
      testPin: 'Test',
      reset: 'Back to auto',
    ...
    /**
     * A coarse token count: exact below a thousand, then one decimal of k/M.
     *
     * A window total is read as a magnitude, so the digits past the leading three
     * carry no information and only make the row harder to scan.
     */
    function formatTokens(value) {
      const n = Number(value ?? 0)
      if (!Number.isFinite(n) || n <= 0) return '0'
      if (n < 1000) return String(Math.round(n))
      // Rounding at the k step can reach 1000k (999,500 -> "1000k"), which is a
      // value the next unit already names. Promote instead of printing it.
      if (n < 1_000_000) {
        const k = Number((n / 1000).toFixed(n < 10_000 ? 1 : 0))
        return k >= 1000 ? `${(n / 1_000_000).toFixed(2)}M` : `${k}k`
      }
    ...
    /** A micro-USD amount as a dollar string. */
    function formatMoney(microUsd) {
      return formatUsd(Number(microUsd ?? 0) / MICRO_USD)
    }
    ...
    class PanelController {
      ...
      inject() {
        ...
        return {
          ...
          loadPlan: (explicit) => controller.loadPlan(explicit),
          loadUsageWindows: (explicit) => controller.loadUsageWindows(explicit),
      ...
      /**
       * Read the official quota windows.
       *
       * The host resolves the key and rewrites the cache, so the returning
       * payload is the whole panel state with the new reading already in it.
       */
      ...
      /**
       * Read the plan's USD cap behind each quota percentage.
       *
       * One cheap request, and the only way a percentage acquires a scale.
       */
      async loadPlan(explicit = false) {
        const call = async () => await this.call('usage.plan', {})
        const result = explicit === true ? await this.run('plan', call) : await this.readQuietly('plan', call)
        if (result !== undefined) this.applyState(result)
        return result
      }

      /**
       * Measure what each window actually cost and consumed.
       *
       * A percentage of an unknown total says nothing, so this is what turns the
       * quota bars into money and tokens; it is a separate, cached reading because
       * it reads the account's usage history.
       */
      async loadUsageWindows(explicit = false) {
        const call = async () => await this.call('usage.windows', {})
        const result = explicit === true ? await this.run('windows', call) : await this.readQuietly('windows', call)
        if (result !== undefined) this.applyState(result)
        return result
      }
    ...
    function ModelRow(props) {
      ...
      return h('div', { className: `cp-model${hidden ? ' cp-model-hidden' : ''}` }, [
        ...
        open ? h('div', { key: 'detail', className: 'cp-panel' }, [
          ...
          // The row's primary button is one click that does four things; saying
          // which four is cheaper than making a user discover them.
          h('div', { key: 'hint', className: 'cp-muted' }, t('autoSetupHint')),
    ...
    function AccountRow(props) {
      const { t, account, disabled, active, pooled, actions } = props
    ...
    function HistoryCard(props) {
      ...
      const failures = entries === null ? 0 : entries.filter((entry) => String(entry.error ?? '') !== '').length
      const summary = entries === null
        ? t('historyScope')
        : `${failures === 0
          ? fill(t('historyStatsClean'), { n: entries.length })
          : fill(t('historyStats'), { n: entries.length, ok: entries.length - failures, bad: failures })} · ${t('historyScope')}`
      return h('div', { className: 'cp-panel' }, [
        h('div', { key: 'bar', className: 'cp-row' }, [
          h('span', { key: 'stats', className: 'cp-muted cp-grow' }, summary),
     ...
     *
     * Two numbers per window, and they answer different questions:
     *
     * - `$used / $cap` comes from the official percentage and the plan's USD cap,
     *   so it is exact and always available. It is what turns "4%" into a size.
     * - The measured line comes from the usage history on demand. It is the only
     *   INDEPENDENT reading, and it says how it was obtained, because the two
     *   differ: a window read per day cannot separate one mid-day boundary from
     *   the day it falls in.
     */
    function UsageCard(props) {
      const { t, usage, plan, detail, disabled, actions, unavailable } = props
      const fetched = Number(usage?.fetchedAt ?? 0) > 0
      const accounts = usage?.accounts ?? []
      const measured = Number(detail?.fetchedAt ?? 0) > 0
      const planOf = (name) => (plan?.accounts ?? []).find((entry) => entry.account === name)
      const detailOf = (name) => (measured ? (detail?.accounts ?? []).find((entry) => entry.account === name) : undefined)
      const anyWindow = accounts.some((entry) => (entry.limits ?? []).length > 0)
      /**
       * One window row for one account.
       *
       * Two numbers, and they answer different questions: `$used / $cap` comes
       * from the official percentage and the plan's USD cap, so it is exact and
       * always available; the measured line comes from the usage history on
       * demand and says how it was obtained, because a window summed per day
       * cannot separate one mid-day boundary from the day it falls in.
       */
      const windowRow = (account, type, key, limit) => {
        const percent = Math.max(0, Math.min(100, Number(limit.percentUsed ?? 0)))
        const left = countdown(t, limit.resetsAt)
        const cap = Number(planOf(account.account)?.caps?.[type] ?? 0)
        const spend = cap > 0 ? (percent / 100) * cap : null
        const row = (detailOf(account.account)?.windows ?? []).find((entry) => String(entry.type) === type)
        /**
         * An empty measurement is only believable when the window is also empty.
         *
         * A reading of "0 tokens, $0.00" is a claim that something was measured,
         * so rendering it beside an official 25% would state a fact that is not
         * true — the gateway answered with no rows, which is a failed measurement,
         * not a zero one. The two cases are told apart by the percentage, which
         * comes from a different endpoint and is always populated.
         */
        const measuredNothing = row !== undefined
          && row.tokens === 0
          && Number(row.costUsd ?? 0) === 0
          && Number(row.requests ?? 0) === 0
          && Number(row.days ?? 0) === 0
        const missingDetail = measuredNothing && percent > 0
        return h('div', { key: type, className: 'cp-usage-item' }, [
          ...
          h('div', { key: 'spend', className: 'cp-usage-tokens', title: t('usageSpendHint') },
            spend === null
              ? t('usageNoPlan')
              : fill(t('usageSpend'), { used: formatMoney(spend), cap: formatMoney(cap) })),
          missingDetail
            ? h('div', { key: 'missing', className: 'cp-usage-note cp-muted' }, t('usageNoDetail'))
            : row === undefined
              ? null
              : h('div', { key: 'measured', className: 'cp-usage-note', title: t('usageMeasuredHint') }, [
                h('span', { key: 'v' }, fill(t('usageMeasured'), {
                  cost: formatMoney(row.costUsd),
                  tokens: formatTokens(row.tokens),
                  // A window summed per day reports days, not requests: saying
                  // "8 requests" for eight days would be a plain lie.
                  n: Number(row.requests ?? 0) > 0
                    ? fill(t('usageRequests'), { n: Number(row.requests) })
                    : fill(t('usageDaysSummed'), { n: Number(row.days ?? 0) }),
                })),
                h('span', { key: 'how', className: 'cp-muted' }, ` · ${row.exact === true ? t('usageExact') : t('usageDaily')}`),
              ]),
          left === '' ? null : h('div', { key: 'note', className: 'cp-usage-note' }, fill(t('usageResetsIn'), { t: left })),
          // Which models the window's spend went to. A percentage says how much
          // is gone; this says what took it, which is the part a user can act on.
          missingDetail || row === undefined || (row.models ?? []).length === 0
            ? null
            : h('details', { key: 'models', className: 'cp-usage-models' }, [
              h('summary', { key: 'sum', className: 'cp-usage-models-sum' },
                `${t('usageByModel')} · ${Number(row.modelCount ?? row.models.length) > row.models.length
                  ? fill(t('usageByModelTop'), { n: row.models.length, total: Number(row.modelCount) })
                  : String(row.models.length)}`),
              h('div', { key: 'rows', className: 'cp-usage-model-rows' }, row.models.map((entry) => h('div', {
                key: entry.model, className: 'cp-usage-model-row', title: entry.model,
              }, [
                // The route prefix is constant across every row while the quota columns
                // are narrow — the same reason the history table strips it. The full
                // id stays in the tooltip.
                h('span', { key: 'name', className: 'cp-usage-model-name', title: entry.model }, String(entry.model).replace(/^cline-pass\//, '')),
                h('span', { key: 'cost', className: 'cp-usage-model-cost' }, formatMoney(entry.costUsd)),
                h('span', { key: 'tokens', className: 'cp-usage-model-tokens' }, formatTokens(entry.tokens)),
              ]))),
            ]),
        ])
      }
      /** One account's block: its name when there is more than one, then its windows. */
      const accountGroup = (account) => {
        ...
        const inner = account.ok !== true
          ...
          : windows.length === 0
            ...
            : h('div', { key: 'bars', className: 'cp-usage' }, windows.map(([type, key, limit]) => windowRow(account, type, key, limit)))
        return h('div', { key: account.account, className: 'cp-usage-group' }, [
          accounts.length > 1 ? h('div', { key: 'name', className: 'cp-usage-account' }, account.account) : null,
          inner,
        ])
      }
      let body
      ...
      else body = h('div', { key: 'groups', className: 'cp-usage-groups' }, accounts.map(accountGroup))
      ...
      return h('div', { className: 'cp-card' }, [
        h('div', { key: 'title', className: 'cp-card-title' }, [
          ...
          h('span', { key: 'btns', className: 'cp-inline' }, [
            h('button', {
              key: 'tk', type: 'button', className: 'cp-btn', disabled: disabled || !anyWindow,
              title: t('usageMeasure'),
              onClick: () => actions.loadUsageWindows(true),
            }, props.counting ? t('usageMeasuring') : t('usageMeasure')),
    ...
    function Panel(props) {
      ...
      // The plan is one more cheap request, and what gives the percentages a
      // scale; it changes when the subscription does, so it is read once per mount.
      React.useEffect(() => {
        if (planAsked.current || state.data === null || state.data.ready !== true) return
        planAsked.current = true
        void props.loadPlan()
      }, [state.data])

      // The window measurement reads the account's usage history, so it is cached
      // and re-taken only when the cached reading has aged out.
      React.useEffect(() => {
        const anyWindow = (state.data?.usage?.accounts ?? []).some((entry) => (entry.limits ?? []).length > 0)
        const read = Number(state.data?.usageWindows?.fetchedAt ?? 0)
        if (state.data === null || !anyWindow) return
        if (read !== 0 && Date.now() - read < TOKEN_TTL_MS) return
        if (windowsAsked.current === read && read !== 0) return
        windowsAsked.current = read
        void props.loadUsageWindows()
      }, [state.data])
      ...
      const actions = {
        ...
        loadPlan: (explicit) => props.loadPlan(explicit),
        loadUsageWindows: (explicit) => props.loadUsageWindows(explicit),
      ...
      return h('div', { className: 'cp-panel' }, [
        ...
        data === null || !ready ? null : h(UsageCard, {
          ...
          usage: data.usage,
          plan: data.plan,
          detail: data.usageWindows,
          counting: state.busy === 'windows',
