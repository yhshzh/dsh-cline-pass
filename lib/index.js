 ...
 *
 * Declared volatile as a whole, the way `llm-pi-ai` declares its `providers`
 * dict: from dsh 0.1.6 the settings service refuses every write to an entry
 * with no volatile field (`Plugin entry "…" has no volatile fields`), so
 * without this the panel's saves — a pin, a hidden model, a stored key's
 * account — are all rejected while its reads keep working, which reads as
 * "the buttons do nothing". Nothing here needs a remount to take effect:
 * every consumer reads through the `current()` source thunk below, so a write
 * reaches the next request without re-registering anything. The declaration is
 * harmless on 0.1.5 and earlier, where the concept does not exist.
 */
