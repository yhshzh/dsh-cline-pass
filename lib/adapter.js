...
/**
 * Default number of streams one account keeps open to the gateway at once.
 *
 * Twice dsh 0.1.7's default `maxActiveSubagents` (8): a main agent with a full
 * set of subagents, plus a title or compaction call, never queues, while a
 * runaway fan-out still cannot open an unbounded number of upstream streams
 * (CWE-770). `0` lifts the cap.
 */
...
//#region upstream concurrency

/**
 * Per-account slots for streams open to the gateway.
 *
 * A slot is held for a whole attempt — from before `fetch` until the response
 * body is closed — because the stream, not the wait for headers, is what keeps
 * a socket and the account's concurrency busy. Accounts are counted apart, so a
 * stalled account cannot hold up requests routed to a healthy one; absorbing
 * exactly that is what the pool is for.
 *
 * Waiters are served first in, first out, and a freed slot is handed to the
 * next waiter and counted in the same step, so a request arriving in between
 * cannot take it as well. A waiter whose signal aborts leaves the queue at
 * once; a held slot is also freed when its signal aborts, so a consumer that
 * stops reading without closing the stream does not keep it.
 *
 * @returns `{ acquire(key, limit, signal) }`, resolving to a release function
 *   that is safe to call more than once. A `limit` below 1 means no limit.
 */
export function createUpstreamSlots() {
  const pools = new Map()
  const admit = (pool) => {
    while (pool.waiting.length > 0 && pool.active < pool.limit) {
      pool.active += 1
      pool.waiting.shift()()
    }
  }
  const hold = (pool, signal) => {
    let held = true
    const release = () => {
      if (!held) return
      held = false
  ...
  return {
    async acquire(key, limit, signal) {
      ...
      if (pool === undefined) {
        pool = { active: 0, limit: Infinity, waiting: [] }
        pools.set(key, pool)
      }
      // Read on every call, so a changed setting reaches the next request; a
      // raised limit admits the queue before this caller joins it.
      pool.limit = Number.isFinite(limit) && limit >= 1 ? Math.floor(limit) : Infinity
      admit(pool)
      if (pool.waiting.length === 0 && pool.active < pool.limit) {
        pool.active += 1
        return hold(pool, signal)
      }
      return await new Promise((resolve, reject) => {
        const grant = () => {
          signal?.removeEventListener('abort', abandon)
          resolve(hold(pool, signal))
        }
        const abandon = () => {
          const at = pool.waiting.indexOf(grant)
          if (at !== -1) pool.waiting.splice(at, 1)
...
export class ClinePassAdapter extends LlmAdapter {
  constructor(config) {
    super()
    this.config = config
    this.upstreamSlots = createUpstreamSlots()
  }
  ...
  resolveModel(provider, model) {
    ...
    // What the last official scan published about this model. Without it a model
    // newer than this release resolves to the `text` fallback and the harness
    // refuses every image for it, silently.
    const published = this.config.publishedModel?.(model) ?? {}
  ...
  async *run(options) {
    ...
    for (const attempt of attempts) {
      ...
      // The account's slot is taken before the attempt's clock starts: the wait
      // is local, and counting it would teach the channel a latency it never had.
      let release
      try {
        release = await this.upstreamSlots.acquire(account.name, connection.maxConcurrentRequests ?? DEFAULT_MAX_CONCURRENT_REQUESTS, upstream)
      } catch (error) {
      ...
      try {
        ...
        // Held until the attempt ends, response body included, so the slot
        // bounds open streams rather than requests waiting for headers.
        release()
