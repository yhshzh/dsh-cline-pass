...
/**
 * Upper bound on requests concurrently in flight to LLM upstreams.
 *
 * Without this, an authenticated caller can fan out unbounded concurrent
 * requests (CWE-770), exhausting sockets/memory on this process and on the
 * upstream accounts. Extra requests simply queue for a free slot.
 */
const MAX_CONCURRENT_UPSTREAM_REQUESTS = 8
let activeUpstreamRequests = 0
const upstreamRequestQueue = []
async function acquireUpstreamSlot() {
  if (activeUpstreamRequests >= MAX_CONCURRENT_UPSTREAM_REQUESTS) {
    await new Promise((resolve) => upstreamRequestQueue.push(resolve))
  }
  activeUpstreamRequests++
}
function releaseUpstreamSlot() {
  activeUpstreamRequests--
  upstreamRequestQueue.shift()?.()
}
/**
