...
export function createStore({ historyLimit = DEFAULT_HISTORY_LIMIT } = {}) {
  ...
  let usageWindows = { ok: false, accounts: [], fetchedAt: 0 }
  /** Last plan reading, one entry per enabled account: the USD cap per window. */
  let plan = { ok: false, accounts: [], fetchedAt: 0 }
  ...
  return {
    ...
    /** The last per-window usage reading (never undefined). */
    usageWindows() {
      return usageWindows
    },
    /** Replace the cached per-window usage reading. */
    setUsageWindows(next) {
      usageWindows = { ...usageWindows, ...next, fetchedAt: Date.now() }
      return usageWindows
    },
    /** The last plan reading — the USD cap behind each window (never undefined). */
    plan() {
      return plan
    },
    /** Replace the cached plan reading. */
    setPlan(next) {
      plan = { ...plan, ...next, fetchedAt: Date.now() }
      return plan
    },
  }
}
